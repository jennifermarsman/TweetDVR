        ...
        public void WriteToHBase()
        {
            ...
            if (cachedTuples.Count > 0)
            {
                ...
                foreach (var cachedTuple in cachedTuples)
                {
                   ...
                   Context.Logger.Info("JEN here is the tuple. Key: " + cachedTuple.Key + " Value: " + cachedTuple.Value);
                ...
                //row.values.Add(new Cell
                //{
                //    column = Encoding.UTF8.scan 'tweet_dvr', { LIMIT => 1 }GetBytes("d:screen_name"),
                //    data = Encoding.UTF8.GetBytes(tweet.Creator.ScreenName)
                //});
                //row.values.Add(new Cell
                //{
                ...
                try
                {
                    ...
                    Context.Logger.Info("You have written {0} rows", writeSet.rows.Count);
                }
                catch
                {
